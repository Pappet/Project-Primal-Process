...
>
> **Triage 2026-08-30 (Direktor):** 🔴 B08 → PLAN-Task (kleiner Feedback-Fix, keine Freigabe nötig). 🟡 Pebble-Munition (27.08.) → **PLAN-Task „Munitions-Ökonomie"** — kollektiv stilles Stack-Verschwinden ist Bug-Klasse, nicht Geschmack. 🔵 Rückzug-Trigger-Task → ersetzt durch Feuer-Ökonomie-Ansatz (Befund Dev 28.08.: Trigger ist der falsche Hebel). 🔵 **Direktor-Flag discovery_gap 0.70 aufgelöst:** dritte Lesung über Band in Folge (0.6→0.65→0.70); Band (0.2–0.6) BLEIBT, keine Abschwächung — die Antwort ist spiel-seitig (PLAN-Ziel 1), die gelockten Wächter-Tests (≤ 0.70) werden zurückgesetzt, sobald die Antwort eine Play-Lesung zeigt (PLAN-Task). Befund-Klasse: jedes neue Druck-System (Kälte, Verletzung, Verschleiß) dämpft die naive Rate — die Ehrlichkeit der Lesungen (nach REC-002/B09) ist inzwischen hoch, das Signal ist echt. Spec-STATUS-Zeilen nachgezogen (SPEC-003/004/005/006/007/008/009/010/011 erledigt). Rohmaterial-Ideen (Save/Load, Weltkarte, persistente Welt) bleiben Ideen-Pool — Promotion nur metrik-getrieben, session_depth ist bis 08.09. in Probezeit.
>
> **Triage 2026-09-20 (Direktor):** 🟡 fire_home_loyalty-Proposal (SPEC-016) → **ANGENOMMEN** (verifiziert: Band 0.3–0.8, Bot liest die Rückkehr-Achse, Detail-Zähler als Diagnose-Block) — Aufnahme in METRICS durch Dev im nächsten Dev-Lauf, `probation_until` 2026-10-04, Präzedenz rest_adoption 13.09. → PLAN-Task T1. 🟡 Menü-Pfad-Lesung post T1/T2/SPEC-014/016 → **PLAN-Task T3** (B10-Menürest: Play 18.09. ruft stoke_fire direkt, Menü-Spieler ungelesen). 🔵 naive_menu.py 4. /tmp-Rebuild (18.09.) → **PLAN-Task T4** (Repo-Datei, Play-Messwerkzeug). ⚪ WARMTH_SEEDS-Löschung (16.09.) bleibt offen — scorecard.py-Touch liegt außerhalb des Direktor-Mandats, Peters Freigabe nötig. rest_adoption-Ziel bleibt bis 24.09. gesperrt. PLAN.md 20.09. neu gefasst (3 Sektionen).
>
> **Triage 2026-09-13 (Direktor):** 🔴 **B11 → PLAN-Task T1** (Feuer-Komfort-Cutoff, Engine-Konstante — Design-Skizze vor TDD, Work-Contract in `.hermes/plans/2026-09-13_180400-direktor-nacht-bogen-t1-t2.md`). 🔴 **B10 → PLAN-Task T2 (Direktor-Entscheid GO):** `stick` → +WOOD (Data-Touch, ein Tag) — die Nacht braucht ~64 Fuel-Ticks, KINDLING-Quellen tragen das nicht, der WOOD-Pfad existiert in der Engine, der Ast ist das naheliegende Holz; kein Blueprint liest WOOD (verifiziert). Tier-2-Anschluss-Andeutung (🟡 07.09.) **VERWORFEN** — sharpen_tool-Präzedenz (5 Lesungen in Folge 0/20, zuletzt 09.09.): Andeutung an einem toten Grundpfad ist Reihenfolge-falsch; bleibt als unverbrauchte Idee, wird nicht getragen. rest_adoption (🟡 11.09.) → METRICS-Aufnahme mit `probation_until` 2026-09-24 + PLAN-Beobachtungszeile (Aufnahme = Dev-Adoption im nächsten Dev-Lauf; kein Play-Nachcommit, Präzedenz 11.09.). Session-Reset-Klasse-Probes (🟡 09.09./07.09. naive_menu.py) bleiben offen — Play-Messwerkzeug, entbürokratisiert die Probes, braucht aber den Play-Job selbst (unantastbar). Design-Fragen aus B10-Play-Reports beantwortet: „warmes Rückzugsziel vs. Feuer"-Textfrage erledigt sich durch den T2-Fuel-Pfad (Antwortpfad statt neuer Text).
...
- [2026-09-14] (Play) **Re-Verifikation auf Tages-HEAD 1ab6d73: 20/20 Tode,
  ALLE exakt @Rest#34, bt_end einheitlich 43.3** (Vorwoche 19/20, Range 33–57).
  Schmaler und deterministischer als der Erstbefund: der Tod kommt nach ~34
...
- [2026-09-16] (Dev) **`WARMTH_SEEDS` in tools/scorecard.py:836 ist toter Code — definiert, nie gelesen** (bei der REST_SEEDS-Bereinigung 16.09. aufgefallen: die analoge `REST_SEEDS`-Konstante des rest_adoption-Laufs behauptete im Kommentar Kanal-Trennung, die nicht existiert — die Runner werden ausschließlich über `_aggregate` mit den Standard-SEEDS geseedet; REST_SEEDS ist entfernt, Kommentar korrigiert). `WARMTH_SEEDS` leidet am selben Muster aus dem SPEC-007-Lauf. Entfernen ist metrik-neutral (kein Lesepfad berührt die Konstante), aber scorecard.py-Anfassung = Metrik-Tooling — Direktor-Freigabe für den Lösch-Commit, kein Verhaltens-Risiko.

- [2026-09-02] (Play) ~~**`play/guided_full.py`: prop-Prozessliste versucht `sharpen_tool` nie — 0/20 Seeds (Sweep 02.09.); auch `treat_cut`/`treat_strain` nie.** gear_uptime (0.994, über Band) ist damit doppelt unsichtbar: der naive Scorecard-Bot hält nie Attrition, und der guided Bot zeigt nie die Gegenmechanik (sharpen selbst spielt sauber: 0.26→0.76, +0.5, 1 flint verbraucht — verifiziert 02.09.). Fix-Richtung: prop-Liste um `sharpen_tool` (+ ggf. treat_*) ergänzen, Worn-Tool-Szene im Bot nachbauen, 20-Sweep gegengetestet (Exhaustion kann minimal steigen = ehrlichere Decke, kein Spiel-Signal). Play-Messwerkzeug, keine Metrik.~~ — **teilerledigt** im Nachcommit des abgebrochenen Play-Laufs 04.09.: 2b-Lücken-Schritt mit Gate (Worn-Tool + Flint, nur solange unbekannt — kein NO_WORN_TOOL-Spin) + Re-Entrancy-Guard in `_treat_if_injured` (gather_tag→_warm_here→treat-Zyklus, RecursionError Seed 20260810). Die künstliche Worn-Tool-Szene (Gipfel-Trips + Wear-Provozieren) war im 20-Sweep **strikte schlechter** (17/20 Tode vs. 12 Baseline, Exhaustion-Median 10 vs. 19.5) und wurde nach Plan-Rückroll-Regel entfernt. sharpen_tool bleibt 0/20 (xfail dokumentiert) — die Koinzidenz Worn-Tool+Flint fällt im natürlichen Verlauf nie zusammen: echtes Spiel-Signal (SPEC-011-Gegenmechanik für Spieler praktisch unentdeckbar), nicht mehr Messwerkzeug-Bug.
- [2026-08-21] (Play) **`play/guided_full.py`: Rückzug-Trigger `not fire_active` blockt den Kälte-Retreat am kalten Ort mit „aktivem aber ungenügendem Feuer".** Am `mountain_peak` zündet der Bot Feuer (fire_active=True) → Rückzug greift nie, obwohl body_temp ~31 bleibt und ~3 HP/Aktion durch Kälte verbluten. Manifestiert seed 20260808 (WP→−70). Der guided-Bot bleibt damit kälte-fragil (6/20 voll vs. 8/20 am 19.08, gleicher bekannter Zustand). Test-Fix „retreat bei body_temp<35 egal ob Feuer" war strikt schlechter (3/20) — jeder Fix muss chirurgisch sein (body_temp-gated NUR am kalten Ort) und per 20-Sweep gegengecheckt werden. Play-Messwerkzeug, keine Metrik.
  **Befund Dev 28.08.: Trigger allein ist der FALSCHE Hebel — nicht gelandet.** Der chirurgische Fix (body_temp<35 UND Ort kälter als forest_edge; mit/ohne exposure≥0.5-Klammer) war im 20-Sweep **16/20 Tode vs. 14/20 Baseline** (5 Seeds brechen früh: 20260802 10→5 BPs, 20260814 7→2). Grund: **alle 14 Baseline-Tode passieren am WALDRAND** (bt 2.8–26.8, injuries leer) in einer Feuer-Versorgungsspirale: Gipfel-Trip im STORM → Waldrand-Feuer brennt ohne stick-Nachschub ab → FIRE_OUT → start_fire unmöglich (tinder/reeds/sticks leer) → bt-Kollaps → HP-Drain. Auch die Versorgungs-Reparatur in `_fire_at` (sticks/reeds nachsammeln + create_tinder) war strikt schlechter (19/20 — ungeschützte Sammel-Ticks vertiefen die Spirale). Der Zustandsraum (Feuer × Wetter × Energie × Materialien) ist um die Baseline ein fragiles lokales Optimum — jede Intervention verschlechtert. Neuer Ansatz nötig: Feuer-Versorgung als Ökonomie (VOR dem Trip sichern, nicht nach dem Kollaps reparieren) — das ist keine Trigger-Frage mehr.
...
<!-- Session-Einträge hier drunter -->
- [2026-09-04] (Dev) **SPEC-011-Gegenmechanik praktisch unentdeckbar — sharpen_tool 0/20 auch mit korrektem Mess-Bot.** Der Play-Lauf 04.09. baute eine künstliche Worn-Tool-Szene (Gipfel-Flint + Wear-Provozieren via log_oak); sie war im 20-Sweep strikte schlechter (17/20 Tode vs. 12 Baseline, Exhaustion-Median 10 vs. 19.5) und wurde zurückgerollt. Der korrekte Bot (2b-Gate: Worn-Tool + Flint, nur solange Prozess unbekannt) trifft die Koinzidenz im natürlichen Verlauf **nie** — Spieler erleben Wear-Warnung (Crossing 0.25), ohne dass Flint im Inventar liegt, und halten Flint, ohne dass ein Werkzeug unter Volllast ist. sharpen selbst funktioniert einwandfrei (0.26→0.76, 1 Flint, verifiziert 02.09.). Spiel-Signal-Klasse wie 13.08. („Extrem-Kälte hat keine richtungsgebende Andeutung"): die Gegenmechanik braucht eine spiel-seitige Brücke (z. B. Wear-Warnung erwähnt Schärfmöglichkeit generisch, oder flint-Nodes wärmer legen). Kein Rezept-Leak, kein Reason-Code-Eingriff. → Direktor-Triage; Details + Zahlen: BACKLOG 🔵 02.09. (teilerledigt) + JOURNAL 04.09.
  **Triage Direktor 06.09.: → PLAN-Task „Ziel-2-Hebel: Wear-Warnung als richtungsgebende Andeutung"** (spiel-seitig, kein Leak, Design-Skizze vor TDD).
- [2026-09-03] (Dev) **SPEC-013 „Jahreszeiten & Wetterfronten": Go/No-Go-Probe auf dem eingereichten
  ...
  **Triage Direktor 06.09.: SPEC-013 VERWORFEN** — der Umfassungs-Vorschlag (Akzeptanz neu fassen)
  wird nicht gezogen: der erwartete Scorecard-Shift ohne spiel-seitigen Grund ist Messverwirrung in einer
  Phase, die gerade Re-Baseline-Probezeiten abarbeitet. Plan archiviert:
  `specs/archived/2026-09-03-SPEC-013-jahreszeiten-wetterfronten-PLAN-NOGO.md`. Thema (Jahreszeiten/
  Wetterfronten) bleibt im Explore-Feld; Neuanlauf nur mit Peters Freigabe, DASS der Stream sich
  verschieben darf (Mess-Systems-Entscheid).
