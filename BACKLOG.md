...
- [2026-08-05] (Dev) **Stack-Verschmelzung vs. Mehrfach-Slot-Crafting:** `Inventory.add` verschmilzt gleichnamige Items → Zwei-Slot-Blueprints, die 2× dasselbe Material brauchen, sind nur über zwei DISTINKTE Materialien erreichbar (Speer = reeds+Ast statt 2×Ast; 2× Stick ist im CLI nie als zwei separate Items auswählbar). **Sauberer Fix (später):** mengenbasiertes Matching in `execute_experiment` (Stack mit quantity N kann N Slots füllen) oder Merge-Logik entschärfen. → Design-Entscheidung für Direktor. — `craft_variety`/`session_depth`.
