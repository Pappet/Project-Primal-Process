...
- [2026-08-17] (Play) **`play/guided_full.py`: `cook_meat` wird nur in 5/20 Seeds als Prozess entdeckt — der Bot isst das rohe Fleisch selbst (via `eat()` auf EDIBLE raw_meat), bevor er es kochen kann.** Der 5. Prozess wird dadurch systematisch unterrepräsentiert (von 20 Seeds erreichen ihn nur 5; `make_fur_cloak` braucht ebenfalls rohes Fleisch, überlebt aber weil er 17/20 erreicht). Kein Spiel-Bug — ein echter Spieler, der Kochen als Ziel kennt, spielt es voll. Fix-Richtung (Play-Messwerkzeug, keine Metrik): beim Jagen rohes Fleisch für `cook_meat` VORBEHALTEN (nicht essen), Kochen vor/nach dem Fell-Nähen in die Prozessreihenfolge einreihen. Nur relevant, damit die Entdeckungs-Decke den 5. Prozess ehrlich misst — ändert die Langeweile-Stelle nicht (die ist Content-Layer-Leere, nicht Bot-Fragilität).
