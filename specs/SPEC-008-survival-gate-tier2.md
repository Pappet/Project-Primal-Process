...
1. **`data/blueprints.json` — kleiner Tier-2-Satz (System-Beweis, nicht Content-Ballon).** 2–3 Blueprints, die die bestehende Rohstoff-/Prozesskette nach vorne führen, mit `min_survival_req` 0.4/0.6 (≈ 2–3 entdeckte Tier-1-Blueprints). Konkrete Vorschläge (Balance entscheidet Dev/Direktor, der Spec definiert das **System**):
   ...
   - `cord_spear` („Seilgebundener Speer“, Stufe über `spear_bound`): `{"tip": "SHARP_OR_RIGID", "shaft": "RIGID", "bind": "FIBER"}`, `min_survival_req: 0.6`, `tool_tags: ["PIERCE"]` → baut auf Tier-1-Speer auf.
...
2. **`data/items.json`:** neue `result_template_id`-Templates für die Tier-2-Ergebnisse (kein dangling) → `content_reachable` bleibt 1.0. Craftable-only Items sind unproblematisch: `content_reachable` zählt nur gather-IDs ∪ process-outputs, keine Blueprint-Items (die 8 Bestands-Werkzeuge sind dort ebenfalls nicht gelistet).
